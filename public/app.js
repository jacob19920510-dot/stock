...
function quoteMap(){ return new Map([...(latest?.groups||[]).flatMap(g=>g.quotes||[]),...(latest?.watchlists||[]).flatMap(g=>g.quotes||[]),...(globalMarketState?.slots||[]).filter(Boolean),...(marketWindState?.slots||[]).filter(Boolean),...(currencyState?.quotes||[]).filter(Boolean)].map(q=>[q.symbol||q.code,q])); }
function optionMap(){ return new Map([...(globalMarketState?.options||[]),...(globalMarketOptions||[])].map(q=>[q.symbol,q])); }
...
function loadDashboardSideData(){
  const marketPromise=fetchJson('/api/global-market').then(data=>{ globalMarketState=data; if(isDashboardModule()){ renderGlobalMarket(); requestAnimationFrame(drawMiniCharts); } return data; }).catch(console.error);
  const windPromise=fetchJson('/api/market-wind').then(data=>{ marketWindState=data; if(isDashboardModule()){ renderWind(); requestAnimationFrame(drawMiniCharts); } return data; }).catch(console.error);
