...
<body>
  ...
  <script>
    ...
    document.body.addEventListener('click',async e=>{ const add=e.target.closest('[data-add]'); if(add){e.stopPropagation(); await post('/api/watchlist/add',{symbol:add.dataset.add,name:add.dataset.name,type:add.dataset.type}); await load(); if(detailData)renderDetail(); if(searchEl.value.trim())search(); else suggestEl.innerHTML=''; return;} const rem=e.target.closest('[data-remove]'); if(rem){e.stopPropagation(); await post('/api/watchlist/remove',{symbol:rem.dataset.remove}); await load(); if(detailData)renderDetail(); if(searchEl.value.trim())search(); return;} if(e.target===detailLayer){closeDetail();return;} const close=e.target.closest('[data-close-detail]'); if(close){closeDetail();return;} const tab=e.target.closest('[data-chart]'); if(tab){chartMode=tab.dataset.chart;if(tab.dataset.period)kPeriod=tab.dataset.period;renderDetail();return;} const open=e.target.closest('[data-open]'); if(open)openDetail(open.dataset.open,open.dataset.name,open.dataset.type,open); });
    refreshButton.addEventListener('click', refreshNow); themeButton.addEventListener('click',()=>setTheme(document.body.classList.contains('light')?'dark':'light')); searchEl.addEventListener('input',()=>{clearTimeout(searchTimer);searchTimer=setTimeout(search,250);}); searchEl.addEventListener('focus',()=>{if(searchEl.value.trim())search();}); marketEl.addEventListener('change',search); document.addEventListener('click',e=>{if(!e.target.closest('.search'))suggestEl.innerHTML='';},true); window.addEventListener('keydown',e=>{if(e.key==='Escape')closeDetail();}); window.addEventListener('resize',()=>{if(detailData)drawChart();}); setTheme(localStorage.getItem('theme')||'dark'); load();
  </script>
</body>
</html>`;
