...
<body>
  ...
  <script>
    ...
    function renderDetail(){ const q=detailData.quote,s=detailData.stats; detailEl.innerHTML='<div class="detail-head"><div class="detail-title"><div><strong>'+esc(q.name)+'</strong> <span class="mono muted">'+esc(q.symbol)+'</span><div class="muted">'+esc(q.type)+' '+esc(q.market)+'</div></div><div><button class="small-btn" data-close-detail>關閉</button></div></div><div class="price">'+fmtNumber(q.price,2)+'</div><div class="'+cls(q.change)+'">'+num(q.change)+' ('+pct(q.changePercent)+')</div></div><div class="tabs"><button data-chart="k" data-period="d" class="'+(chartMode==='k'&&kPeriod==='d'?'active':'')+'">日 K</button><button data-chart="k" data-period="w" class="'+(chartMode==='k'&&kPeriod==='w'?'active':'')+'">週 K</button><button data-chart="k" data-period="m" class="'+(chartMode==='k'&&kPeriod==='m'?'active':'')+'">月 K</button><button data-chart="line" class="'+(chartMode==='line'?'active':'')+'">分時</button></div><div class="chart-wrap"><canvas id="chart"></canvas><div class="chart-tip" id="chartTip"></div></div><div class="legend">'+legend()+'</div><div class="stats">'+stat('開盤',fmtNumber(s.open,2))+stat('最高',fmtNumber(s.high,2))+stat('最低',fmtNumber(s.low,2))+stat('昨收',fmtNumber(s.previousClose,2))+stat('成交量',fmtInt(s.volume))+stat('20日均量',fmtInt(s.avgVolume20))+stat('一年高點',fmtNumber(s.high252||s.fiftyTwoWeekHigh,2))+stat('一年低點',fmtNumber(s.low252||s.fiftyTwoWeekLow,2))+'</div>'; drawChart(); }
